        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (txt_nodes_number.Text.Trim().Length > 0)
                {
                    ...
                    int density = 100;
                    int numofNodes = Convert.ToInt16(txt_nodes_number.Text); // use this in the upcoming for loop instead of density
                    double width = MainWindow.Canvas_SensingFeild.Width;
                    double height = MainWindow.Canvas_SensingFeild.Height;
                    Point center = new Point(width / 2, height / 2);

                    Sensor Sinknode = new Sensor(0);
                    Sinknode.MainWindow = MainWindow;
                    Point sinkPos = new Point(center.X, center.Y);
                    Sinknode.Position = sinkPos;
                    Sinknode.VisualizedRadius = r;
                    MainWindow.Canvas_SensingFeild.Children.Add(Sinknode);
                    MainWindow.myNetWork.Add(Sinknode);

                    Sensor node = new Sensor(1);
