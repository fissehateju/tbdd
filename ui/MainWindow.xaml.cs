...
using TBDD.ExpermentsResults.Energy_consumptions;
using TBDD.ExpermentsResults.Lifetime;
using TBDD.ControlPlane.NOS.TC;
using TBDD.ControlPlane.NOS.TC.subgrapgh;
using TBDD.DataPlane.NeighborsDiscovery;
using TBDD.ui.conts;
using TBDD.ControlPlane.NOS.Visualizating;
    ...
    public partial class MainWindow : Window
    {
        ...
        {           
            if ((PublicParameters.SimulationTime <= stopSimlationWhen + PublicParameters.MacStartUp))
            {
                ...
                Dispatcher.Invoke(() => Title = "TBDD: " + PublicParameters.SimulationTime.ToString(), DispatcherPriority.Send);                
            }           
            else
            {
                TimerCounter.Stop();
        ...
        public void HideSimulationParameters()
        {
            ...
            lbl_Number_of_Delivered_TPacket.Content = "0";
        ...
        private double diameter_optimizer()
        {
            double com = PublicParameters.CommunicationRangeRadius;
            List<double> a = new List<double>() { 1.35, 1.4, 1.45, 1.5, 1.55, 1.6, 1.65, 1.7, 1.75, 1.8, 1.85, 1.9, 1.95, 2 };
            List<double> possible_diam = new List<double>() { 3 * com * a[0], 3 * com * a[1], 3 * com * a[2], 3 * com * a[3], 3 * com * a[4], 3 * com * a[5], 3 * com * a[6],
                3 * com * a[7]};

            double min = double.MaxValue;
            int a_indexer = 0;
            for (int i = 0; i < possible_diam.Count; i++)
            {
                if (Canvas_SensingFeild.ActualWidth % possible_diam[i] <= min)
                {
                    min = Canvas_SensingFeild.ActualWidth % possible_diam[i];
                    a_indexer = i;
                }
            }
            return a[a_indexer];
        ...
        public void RandomDeplayment(int sinkIndex)
        {
            ...
            double parameter_a = diameter_optimizer();
            double diameter = PublicParameters.CommunicationRangeRadius * parameter_a;

            if (Canvas_SensingFeild.ActualWidth == 400) diameter = PublicParameters.CommunicationRangeRadius * 1.35;

            double clusterArea = Math.PI * (Math.Pow((diameter), 2));

            PublicParameters.cellDiameter = diameter;

            PublicParameters.myNetwork = myNetWork;

            _ = new LocalRegion(Canvas_SensingFeild, diameter);
            _ = new NetworkConstruction(Canvas_SensingFeild);
        ...
        public void ClearExperment()
        {
            try
            {
                ...
                PublicParameters.NumberOfGeneratedRegionBeaconPacket = 0;
                PublicParameters.NumberofGeneratedResponsePacket = 0;
                PublicParameters.NumberofGeneratedFollowUpPackets = 0;

                PublicParameters.NumberofDelieveredFollowUpPackets = 0;
                PublicParameters.NumberOfDelieveredDataPackets = 0;
                PublicParameters.NumberOfDelieveredQueryPackets = 0;
                PublicParameters.NumberofDelivedRegBeaconPackets = 0;
                PublicParameters.OverallDelieverdPackets = 0;

                PublicParameters.TotalNumberOfHops = 0;
                PublicParameters.TotalDelayMs = 0;
                PublicParameters.TotalNumberOfHopsControl = 0;
                PublicParameters.TotalNumberOfHopsData = 0;
                PublicParameters.TotalRoutingDistance = 0;
                ...
                PublicParameters.EnergyConsumedForDataPackets = 0;
                PublicParameters.EnergyComsumedForRegAds = 0;
        ...
        void InitliazeSinkTimer_Tick(object sender, EventArgs e)
        {
            ...
            /* ///////////////// 
            ///////// thus two wasn't  under comment and sink was not working. b/c function start moving should be defined under the button click*/
            ///
            mobileModel.startMoving();
            sinkMoving = true;
        }
