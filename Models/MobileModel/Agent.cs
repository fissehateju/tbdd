...
namespace TBDD.Models.MobileModel
{
    public class Agent
    {
        ...
        private void OutOfRangeTimer_Tick(object sender, EventArgs e)
        {
            if (AgentBuffer.Count > 0)
            {
                ...
                else if (Operations.isInMyComunicationRange(Node, PublicParameters.SinkNode))
                {
                    do
                    {
                        Packet packet = AgentBuffer.Dequeue();
            ...
            if (waitingTimesForSink > 7)
            {
                ...
                if (AgentBuffer.Count > 0)
                {
                    ...
                    do
                    {
                        ...
                        packet.aDroppedReason = "WaitingTimesforSink > 7";
                        Node.updateStates(packet);
                    } while (AgentBuffer.Count > 0);
                }
                ...
                else
                {
                    if (NewAgent != null)
                    {
                        ...
                        do
                        {
                            ...
                            packet.TimeToLive += Node.maxHopsForDestination(Node.CenterLocation, NewAgent.CenterLocation);
                            packet.DestinationAddress = packet.Destination.CenterLocation;
                            Node.sendDataPack(packet);
                        } while (AgentBuffer.Count > 0);
