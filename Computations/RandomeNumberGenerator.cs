...
namespace TBDD.Intilization 
{
    ...
    {
        public static double getDirInter()
        {
            double x = GetExponential() % 1;
            x *= 10;
            x %= 5;
            return x;
        }
        public static int uniformMaxSpeed(double max)
        {
            if (max == 0)
            {
                return 0;
            ...
            double value = Math.Floor(GetNormal(max, 2));
            if(value > max * 2)
            {
                value = max * 2;
            }
            ...
            {
                Console.WriteLine("Out of memory in get speed");
                Console.WriteLine(e);
                return 0;
            }
        ...
        public static int uniformMaxDirection(double mean)
        {
            double x = Math.Abs(Math.Floor(GetNormal(mean, 45)));
            x %= 360;
            int number = System.Convert.ToInt16(x);
            // Console.WriteLine("Random Direction is {0}", number);
            return number;
        }

        public static int uniformClusterNumber(double median, int clusterCount)
        {
            int number = System.Convert.ToInt16(Math.Floor(GetNormal(median, 1)));
            number %= clusterCount;
            // Console.WriteLine("Random Speed is: {0}",number);
            return number;
